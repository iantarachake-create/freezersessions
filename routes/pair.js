...
router.get('/', async (req, res) => {
    ...
    async function POPKID_PAIR_CODE() {
    ...
    console.log(version);
        ...
        try {
            ...
            if (!Popkid.authState.creds.registered) {
                await delay(1500);
                num = num.replace(/[^0-9]/g, '');
                
                const randomCode = generateRandomCode();
                const code = await Popkid.requestPairingCode(num, randomCode);
                ...
                if (connection === "open") {
                    ...
                    try {
                        ...
                        while (sendAttempts < maxSendAttempts && !sessionSent) {
                            try {
                                Sess = await sendButtons(Popkid, Popkid.user.id, {
            title: '',
            text: 'FREEZER~' + b64data,
            footer: `> *POWERED BY FREEZER*`,
            buttons: [
                { 
                    name: 'cta_copy', 
                    buttonParamsJson: JSON.stringify({ 
                        display_text: 'Copy Session', 
                        copy_code: 'FREEZER~' + b64data 
                    }) 
                },
                {
                    name: 'cta_url',
                    buttonParamsJson: JSON.stringify({
                        ...
                        url: 'https://github.com/Freezerfred/Freezer-MD-'
                    })
                },
                {
                    name: 'cta_url',
